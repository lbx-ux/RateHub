...
### 1.6 用户今日签到
- **接口名称**：用户签到
- **接口描述**：记录当前登录用户在今日的签到状态（基于 Redis Bitmap）。
- **请求 URL**：`/user/sign`
- **请求方式**：`POST`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
  | `authorization` | `session-token-string` | 是 | 用户登录 Token |
...
### 1.7 统计当月连续签到天数
- **接口名称**：连续签到统计
- **接口描述**：查询当前用户在本月截止到今天为止的连续签到天数。
- **请求 URL**：`/user/sign/count`
- **请求方式**：`GET`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
...
- **请求参数 (Query)**：
  ...
  | `distance` | `Integer` | 否 | `5000` | 距离范围限制，单位为米（如：`1000`, `3000`, `5000`）。为空时代表不限制距离 |
