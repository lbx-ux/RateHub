...
### 1.8 修改个人资料
- **接口名称**：修改资料
- **接口描述**：修改当前登录用户的个人基本资料（昵称、城市、简介等）。
- **请求 URL**：`/user/update`
- **请求方式**：`PUT`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
...
### 1.10 根据用户 ID 查询基本信息
- **接口名称**：根据ID查用户
- **接口描述**：根据用户 ID 获取该用户的脱敏基本信息（常用于评论区、动态列表展示）。
- **请求 URL**：`/user/{id}`
- **请求方式**：`GET`
- **响应数据示例 (成功)**：
  ```json
...
### 1.11 上传用户头像
- **接口名称**：上传头像
- **接口描述**：上传用户的个人头像图片，返回图片存储访问的绝对路径 URL。
- **请求 URL**：`/upload/avatar`
- **请求方式**：`POST`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
  | `Content-Type` | `multipart/form-data` | 是 | 多部分表单格式 |
- **请求参数 (Form Data)**：
  | 参数名 | 类型 | 是否必填 | 示例值 | 说明 |
  | :--- | :--- | :--- | :--- | :--- |
  | `file` | `File` | 是 | `[Binary]` | 头像二进制文件 |
- **响应数据示例 (成功)**：
  ```json
  {
    "code": 200,
    "message": "操作成功",
    "data": "/imgs/avatar/upload_9876.jpg"
  }
  ```
...
### 1.12 统计当月累计签到天数
- **接口名称**：累计签到统计
- **接口描述**：查询当前登录用户在本月内一共签到了多少天。
- **请求 URL**：`/user/sign/total`
- **请求方式**：`GET`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
...
### 2.5 更新商铺信息
- **接口名称**：更新商铺
- **接口描述**：供后台管理系统使用，修改已有商铺的信息。
- **请求 URL**：`/shop`
- **请求方式**：`PUT`
- **请求参数 (Request Body)**：包含商铺完整信息，必须携带 `id`。
- **响应数据示例 (成功)**：
  ```json
  {
    "code": 200,
...
### 2.6 按名称搜索商铺
- **接口名称**：搜索商铺
- **接口描述**：根据商铺名称的关键字分页模糊搜索商铺。
- **请求 URL**：`/shop/of/name`
- **请求方式**：`GET`
- **请求参数 (Query)**：
  | 参数名 | 类型 | 是否必填 | 示例值 | 说明 |
  | :--- | :--- | :--- | :--- | :--- |
  | `name` | `String` | 否 | `西湖` | 搜索关键字 |
  | `current` | `Integer` | 否 | `1` | 当前页码，默认 1 |
- **响应数据示例 (成功)**：与 2.2 接口一致。
...
### 3.3 新增普通优惠券
- **接口名称**：新增普通券
- **接口描述**：供后台管理系统使用，给指定商铺新增一张不限量的普通代金券。
- **请求 URL**：`/voucher`
- **请求方式**：`POST`
- **请求参数 (Request Body)**：
  ```json
...
### 3.4 新增秒杀优惠券
- **接口名称**：新增秒杀券
- **接口描述**：供后台管理系统使用，给指定商铺新增一张限量的秒杀代金券，并同步库存到 Redis。
- **请求 URL**：`/voucher/seckill`
- **请求方式**：`POST`
- **请求参数 (Request Body)**：
  ```json
  {
    ...
    "type": 1,
    "stock": 100,
    "beginTime": "2026-10-01T00:00:00",
    "endTime": "2026-10-07T23:59:59"
  }
  ```
- **响应数据示例 (成功)**：与 3.3 接口一致。
...
- **接口名称**：普通券下单
- **接口描述**：购买不限量的普通代金券。
- **请求 URL**：`/voucher-order/normal/{id}`
- **请求方式**：`POST`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
  | `authorization` | `session-token-string` | 是 | 登录凭证 |
- **请求参数 (Path)**：
  | 参数名 | 类型 | 是否必填 | 示例值 | 说明 |
  | :--- | :--- | :--- | :--- | :--- |
  | `id` | `Long` | 是 | `101` | 优惠券 ID |
- **响应数据示例 (成功)**：返回新建订单的唯一 ID。与 3.2 接口一致。

---

### 3.6 查询我的优惠券订单
- **接口名称**：我的订单
- **接口描述**：查询当前登录用户购买的所有普通优惠券和秒杀优惠券记录。
- **请求 URL**：`/voucher-order/my`
- **请求方式**：`GET`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
...
### 4.10 查询指定用户的探店日记
- **接口名称**：指定用户日记
- **接口描述**：分页查询某个指定用户发布的所有探店日记（用于查看他人主页）。
- **请求 URL**：`/blog/of/user`
- **请求方式**：`GET`
- **请求参数 (Query)**：
  | 参数名 | 类型 | 是否必填 | 示例值 | 说明 |
  | :--- | :--- | :--- | :--- | :--- |
  | `id` | `Long` | 是 | `1001` | 目标用户 ID |
  | `current` | `Integer` | 否 | `1` | 当前页码，默认 1 |
- **响应数据示例 (成功)**：与 4.9 接口一致。

---

### 4.11 删除探店日记
- **接口名称**：删除日记
- **接口描述**：删除当前登录用户自己发布的一篇探店日记。
- **请求 URL**：`/blog/{id}`
- **请求方式**：`DELETE`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
...
### 5.1 发布评论
- **接口名称**：发布评论
- **接口描述**：在指定的探店日记下发布文本评论。
- **请求 URL**：`/blog-comments`
- **请求方式**：`POST`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
  | `authorization` | `session-token-string` | 是 | 登录凭据 |
- **请求参数 (Request Body)**：
  ```json
  {
    "blogId": 501,
...
### 5.2 查询日记下的评论列表
- **接口名称**：查询评论
- **接口描述**：根据日记 ID 获取该日记下的所有评论列表。
- **请求 URL**：`/blog-comments`
- **请求方式**：`GET`
- **请求参数 (Query)**：
  | 参数名 | 类型 | 是否必填 | 示例值 | 说明 |
  | :--- | :--- | :--- | :--- | :--- |
  | `blogId` | `Long` | 是 | `501` | 被评论的探店日记 ID |
- **响应数据示例 (成功)**：
  ```json
  {
    "code": 200,
...
### 6.5 查询我的粉丝列表
- **接口名称**：我的粉丝
- **接口描述**：查询所有关注了当前登录用户的粉丝列表。
- **请求 URL**：`/follow/fans/me`
- **请求方式**：`GET`
- **请求头 (Request Headers)**：
  | 参数名 | 参数值 | 是否必填 | 说明 |
  | :--- | :--- | :--- | :--- |
  | `authorization` | `session-token-string` | 是 | 用户登录 Token |
- **响应数据示例 (成功)**：与 6.4 接口一致。
...
### 8.1 平台全局 UV 统计
- **接口名称**：平台UV统计
- **接口描述**：查询平台的独立访客数（UV），包括今日、本周和近30天的累计 UV。该接口供前端全局展示。
- **请求 URL**：`/uv/stats`
- **请求方式**：`GET`
- **响应数据示例 (成功)**：
  ```json
